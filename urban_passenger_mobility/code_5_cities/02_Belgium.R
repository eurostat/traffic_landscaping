...
## Belgium: Brussels (note different dates)


# download 2022 data from https://opendata.stib-mivb.be/store/ and make an account to download gtfs data 
# pop data from https://hub.worldpop.org/geodata/summary?id=49932


name_country_folder       <- "02_Belgium"
name_country_osm_file     <- "europe-latest.osm.pbf"
name_FUA_osm_file         <- "current_capital_file.osm.pbf"
urban_area_of_interest    <- "BE001K1"
path_to_shape_file        <- "URAU_RG_100K_2020_3857_GREATER_CITIES.shp/URAU_RG_100K_2020_3857_GREATER_CITIES.shp"
name_layer                <- "URAU_RG_100K_2020_3857_GREATER_CITIES"
path_to_gtfs_file         <- "gtfs.zip"
path_to_subset_gtfs_file  <- "current_city_GTFS_file.zip"
path_to_saved_elevation   <- "elevation_current_city.tif"
start_date                <- "2022-08-08"
end_date                  <- "2022-08-14"
population_data_path      <- "bel_ppp_2020_UNadj_constrained.tif"
name_OTP_folder           <- "OTP_folder"
date_and_time             <- as.POSIXct(strptime("2022-08-09 08:30", "%Y-%m-%d %H:%M", tz ="Europe/Amsterdam"))
...
#percent_walk_morning <-  0.9156966

# wheel chair boarding does not excist

all_stops$wheelchair_boarding



accesibility_stats <- tibble(percent_500m, percent_walk,percent_walk_morning, total_pop, country = "Belgium", city = "Brussels")
...
log1 <- otp_build_graph(otp = path_otp, dir =  file.path(name_country_folder,path_data),  memory = 8000)
