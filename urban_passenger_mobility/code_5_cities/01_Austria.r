...
# download 2022 data from https://www.transit.land/feeds/f-u2ed-wienerlinien~wlb/]
# pop data from https://hub.worldpop.org/geodata/summary?id=49932
# more https://mobilitaetsdaten.gv.at/en/daten/soll-fahrplandaten-gtfs


name_country_folder       <- "01_Austria"
name_country_osm_file     <- "europe-latest.osm.pbf"
name_FUA_osm_file         <- "current_capital_file.osm.pbf"
urban_area_of_interest    <- "AT001C1"
...
path_to_gtfs_file         <- "eeed0329a37f4fb190ec832d66af38e9c391851b.zip"
path_to_subset_gtfs_file  <- "current_city_GTFS_file.zip"
path_to_saved_elevation   <- "elevation_current_city.tif"
start_date                <- "2022-05-16"
end_date                  <- "2022-05-22"
population_data_path      <- "aut_ppp_2020_UNadj_constrained.tif"
...
# Load GTFS file
full_gtfs_file <- gtfs2gps::read_gtfs(file.path(name_country_folder,path_to_gtfs_file))
...
subset_gtfs_file <- gtfs2gps::read_gtfs(file.path(name_country_folder,path_to_subset_gtfs_file))
...
#percent_walk_morning <- 0.8081914
...
accesibility_stats <- tibble(percent_500m, percent_walk,percent_walk_morning, total_pop, country = "Austria", city = "Vienna")
