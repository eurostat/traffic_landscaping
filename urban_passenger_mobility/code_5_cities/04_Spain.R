...
## Spain: madrid (note different dates)


# download 2022 data from 
# http://crtm.maps.arcgis.com/sharing/rest/content/items/885399f83408473c8d815e40c5e702b7/data
# https://crtm.maps.arcgis.com/sharing/rest/content/items/aaed26cc0ff64b0c947ac0bc3e033196/data
# https://servicios.emtmadrid.es:8443/gtfs/transitemt.zip
# http://crtm.maps.arcgis.com/sharing/rest/content/items/5c7f2951962540d69ffe8f640d94c246/data
# https://www.arcgis.com/sharing/rest/content/items/868df0e58fca47e79b942902dffd7da0/data

# pop data from https://hub.worldpop.org/geodata/summary?id=49958


name_country_folder       <- "04_Spain"
name_country_osm_file     <- "europe-latest.osm.pbf"
name_FUA_osm_file         <- "current_capital_file.osm.pbf"
urban_area_of_interest    <- "ES001C1"
...
path_to_gtfs_file         <- c("google_transit_M10.zip" , "google_transit_M4.zip","google_transit_M6.zip","google_transit_M89.zip","transitemt.zip")
path_to_subset_gtfs_file  <- "current_city_GTFS_file.zip"
path_to_saved_elevation   <- "elevation_current_city.tif"
start_date                <- "2022-08-08"
end_date                  <- "2022-08-14"
population_data_path      <- "esp_ppp_2020_UNadj_constrained.tif"
...
# Load GTFS file
# gtfs_list <- list(lapply(file.path(name_country_folder,path_to_gtfs_file), gtfstools::read_gtfs))
# full_gtfs_file <- gtfstools::merge_gtfs(list(gtfs_list))

gtfs_1 <- gtfstools::read_gtfs(file.path(name_country_folder,path_to_gtfs_file[1]))
gtfs_2 <- gtfstools::read_gtfs(file.path(name_country_folder,path_to_gtfs_file[2]))
gtfs_3 <- gtfstools::read_gtfs(file.path(name_country_folder,path_to_gtfs_file[3]))
gtfs_4 <- gtfstools::read_gtfs(file.path(name_country_folder,path_to_gtfs_file[4]))
gtfs_5 <- gtfstools::read_gtfs(file.path(name_country_folder,path_to_gtfs_file[5]))

gtfs_list <- list("gtfs_1" = gtfs_1,"gtfs_2" =  gtfs_2, "gtfs_3" = gtfs_3, "gtfs_4" = gtfs_4,"gtfs_5" =  gtfs_5)
#gtfs_list <- list(lapply(file.path(name_country_folder,path_to_gtfs_file), gtfstools::read_gtfs))
full_gtfs_file <- gtfstools::merge_gtfs(gtfs_list)
...
#subset_gtfs_file <- full_gtfs_file
...
gtfstools::write_gtfs(subset_gtfs_file, file.path(name_country_folder,path_to_subset_gtfs_file))
...
# polygon 
percent_500m  <- raster::extract(population_data_city_brick,
...
stops_2_per_weekday_average <- gtfs_pattern$.$dates_servicepatterns %>% 
  ...
  mutate(departure_date_standardized =anytime::anytime(paste0("2022-01-01 ", 
                                                              substr(as.character(departure_date_time), 12, 20)))) %>% 
  mutate(during_7_10_am = departure_date_standardized < ymd_hms(paste0("2022-01-01 09:00:00")) &
...
#percent_walk_morning <-   0.8495073

# wheel chair boarding does  exist
...
accesibility_stats <- tibble(percent_500m, percent_walk,percent_walk_morning, total_pop,wheelchair_accessible, country = "Spain", city = "Madrid")
