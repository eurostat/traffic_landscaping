...
## Croatia: Zagreb (note different dates)


# download 2022 data from https://www.zet.hr/odredbe/datoteke-u-gtfs-formatu/669 
# pop data from https://hub.worldpop.org/geodata/summary?id=49958


name_country_folder       <- "03_Croatia"
name_country_osm_file     <- "europe-latest.osm.pbf"
name_FUA_osm_file         <- "current_capital_file.osm.pbf"
urban_area_of_interest    <- "HR001C1"
...
subset_gtfs_file$trips <- subset_gtfs_file$trips %>%
  filter(route_id   %in% subset_gtfs_file$routes$route_id  )


subset_gtfs_file$stop_times <- subset_gtfs_file$stop_times %>%
  filter(trip_id  %in% subset_gtfs_file$trips$trip_id)

subset_gtfs_file$stops <- subset_gtfs_file$stops %>%
  filter(stop_id    %in% subset_gtfs_file$stop_times$stop_id   )
...
#percent_walk_morning <-   0.8093365
...
accesibility_stats <- tibble(percent_500m, percent_walk,percent_walk_morning, total_pop, country = "Croatia", city = "Zagreb")
